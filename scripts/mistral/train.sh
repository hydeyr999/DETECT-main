...
for domain in ${domains[@]}; do
  ...
  python detectors/mistral/run_mistral.py --task cross-domain --dataset $domain --mode train \
...
for operation in ${operations[@]}; do
  ...
  python detectors/mistral/run_mistral.py --task cross-operation --dataset $operation --mode train \
  --device $device --epochs $epochs --threshold $threshold --n_sample $n_sample
done