...
for domain in ${domains[@]}; do
  ...
  python detectors/deberta/run_deberta.py --task cross-domain --dataset $domain --mode train \
...
for model in ${models[@]}; do
  ...
  python detectors/deberta/run_deberta.py --task cross-model --dataset $model --mode train \
  --device $device --epochs $epochs --threshold $threshold --n_sample $n_sample
done

for operation in ${operations[@]}; do
  echo "Training on $operation, epochs=$epochs, threshold=$threshold, n_sample=$n_sample"
  python detectors/deberta/run_deberta.py --task cross-operation --dataset $operation --mode train \
  --device $device --epochs $epochs --threshold $threshold --n_sample $n_sample
done