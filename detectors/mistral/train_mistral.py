...
try:
    ...
    from mistral.r_detect.ai_loader import AiCollator, AiCollatorTrain, show_batch
    from mistral.r_detect.ai_model import (LlamaForDetectAI, MistralForDetectAI,
                                   PhiForDetectAI)
    from mistral.r_detect.ai_optimizer import get_optimizer
    from mistral.Utils.metric_utils import compute_metrics
    from mistral.Utils.train_utils import AverageMeter, as_minutes, get_lr
...
def clean_utf(text_list):
    text_list = deepcopy(text_list)
    for idx in range(len(text_list)):
        text_list[idx] = text_list[idx].encode('utf-8', 'ignore').decode('utf-8')
    return text_list

# @hydra.main(version_base=None, config_path="../conf/r_detect", config_name="conf_r_detect")
def run_training(cfg,df,device,threshold=0):
    ...
    essay_df = deepcopy(df)
    essay_df = essay_df[~essay_df['text'].isna()].copy()
    essay_df = essay_df.reset_index(drop=True)
    essay_df['text'] = clean_utf(essay_df['text'])

    print(essay_df.head())
    print(essay_df.shape)

    rng = random.Random(cfg.seed)
    essay_df['fold'] = essay_df['text'].apply(lambda x: 'train' if rng.random() < 0.9 else 'valid')
