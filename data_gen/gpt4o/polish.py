...
def get_4o_polish(args,context):
...
def run(args):
    ...
    df = run_4o_polish(args,df)

    print(df.head())
    print(df.shape)
    print(df.columns)

    df.to_csv(f'./data_gen/LLM-texts/polish/{args.dataset}_4o_polish.csv', index=False)
