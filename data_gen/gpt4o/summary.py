...
def get_4o_summary(args, context):
    try:
...
def run_4o_summary(args,df):
    ai_texts = []
    for _, context in tqdm.tqdm(df.iterrows(), total=len(df)):
        context = context.tolist()[0]
        ai_texts.append(get_4o_summary(args, context))
    df['ai'] = ai_texts
    return df

def run(args):
    df = get_df(args)[2900:3000]

    df = run_4o_summary(args, df)

    print(df.head())
    print(df.shape)
    print(df.columns)

    df.to_csv(f'./data_gen/LLM-texts/summary/{args.dataset}_4o_summary.csv', index=False)
