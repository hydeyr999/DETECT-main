...
def get_4o_refine(args, context):
    try:
...
def run_4o_refine(args,df):
    ai_texts = []
    for _, context in tqdm.tqdm(df.iterrows(), total=len(df)):
        context = context.tolist()[0]
        ai_texts.append(get_4o_refine(args, context))
    df['ai'] = ai_texts
    return df

def run(args):
    df = get_df(args)[1400:1500]

    df = run_4o_refine(args, df)

    print(df.head())
    print(df.shape)
    print(df.columns)

    df.to_csv(f'./data_gen/LLM-texts/refine/{args.dataset}_4o_refine.csv', index=False)
