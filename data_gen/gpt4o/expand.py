...
def get_4o_expand(args, context):
    try:
...
def run_4o_expand(args,df):
    ai_texts = []
    for _, context in tqdm.tqdm(df.iterrows(), total=len(df)):
        context = context.tolist()[0]
        ai_texts.append(get_4o_expand(args, context))
    df['ai'] = ai_texts
    return df

def run(args):
    df = get_df(args)[1300:1400]

    df = run_4o_expand(args, df)

    print(df.head())
    print(df.shape)
    print(df.columns)

    df.to_csv(f'./data_gen/LLM-texts/expand/{args.dataset}_4o_expand.csv', index=False)
