...
from llm_prompts import polish_prompts
...
def get_qwen_polish(args,context):

    prompt_template = polish_prompts[args.dataset]
...
def run_qwen_polish(args,df):
    ai_texts = []
    for _, context in tqdm.tqdm(df.iterrows(), total=len(df)):
        context = context.tolist()[0]
        ai_texts.append(get_qwen_polish(args,context))
    df['ai'] = ai_texts
    return df

def run(args):
    df = get_df(args)[2000:2500]

    df = run_qwen_polish(args,df)

    print(df.head())
    print(df.shape)
    print(df.columns)

    df.to_csv(f'./data_gen/LLM-texts/polish/{args.dataset}_Qwen_polish.csv', index=False)
