...
def get_qwen_expand(args,context):
...
def run_qwen_expand(args,df):
    ai_texts = []
    for _, context in tqdm.tqdm(df.iterrows(), total=len(df)):
        context = context.tolist()[0]
        ai_texts.append(get_qwen_expand(args,context))
    df['ai'] = ai_texts
    return df

def run(args):
    df = get_df(args)[2300:2400]

    df = run_qwen_expand(args,df)

    print(df.head())
    print(df.shape)
    print(df.columns)

    df.to_csv(f'./data_gen/LLM-texts/expand/{args.dataset}_Qwen_expand.csv', index=False)
