...
def get_qwen_create(args, context):
...
def run(args):
    ...
    print(df)
    df = run_qwen_create(args, df)

    print(df.head())
    print(df.shape)
    print(df.columns)

    df.to_csv(f'./data_gen/LLM-texts/create/{args.dataset}_Qwen_create.csv', index=False)
