...
def get_qwen_translate(args, context):
    try:
...
def run_qwen_translate(args,df):
    ai_texts = []
    for _, context in tqdm.tqdm(df.iterrows(), total=len(df)):
        context = context.tolist()[0]
        ai_texts.append(get_qwen_translate(args, context))
    df['ai'] = ai_texts
    return df
...
def run(args):
    ...
    df = run_qwen_translate(args, df)

    print(df.head())
    print(df.shape)
    print(df.columns)

    df.to_csv(f'./data_gen/LLM-texts/translate/{args.dataset}_Qwen_translate.csv', index=False)
