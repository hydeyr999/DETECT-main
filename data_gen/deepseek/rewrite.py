...
def get_ds_rewrite(args, context):
    try:
...
def run_ds_rewrite(args,df):
    ai_texts = []
    for _, context in tqdm.tqdm(df.iterrows(), total=len(df)):
        context = context.tolist()[0]
        ai_texts.append(get_ds_rewrite(args, context))
    df['ai'] = ai_texts
    return df

def run(args):
    df = get_df(args)[2800:2900]

    df = run_ds_rewrite(args, df)

    print(df.head())
    print(df.shape)
    print(df.columns)

    df.to_csv(f'./data_gen/LLM-texts/rewrite/{args.dataset}_ds_rewrite.csv', index=False)
