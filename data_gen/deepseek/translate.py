...
def get_ds_translate(args, context):
...
def run(args):
    ...
    df = run_ds_translate(args, df)

    print(df.head())
    print(df.shape)
    print(df.columns)

    df.to_csv(f'./data_gen/LLM-texts/translate/{args.dataset}_ds_translate.csv', index=False)
