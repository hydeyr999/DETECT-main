...
from llm_prompts import rewrite_prompts
...
def get_rewrite(model, tokenizer, context):
    prompt_template = rewrite_prompts[args.dataset]
    prompt = prompt_template.format(context=context)
    response_content = get_response(model, tokenizer, prompt)
    return response_content

def run_rewrite(args,df):
    tokenizer, model = get_aux_llm(args)
    ai_texts = []
    for _, context in tqdm.tqdm(df.iterrows(), total=len(df)):
        context = context.tolist()[0]
        ai_texts.append(get_rewrite(model, tokenizer, context))
    df['ai'] = ai_texts
    return df

def run(args):
    df = get_df(args)[2600:2700]

    df = run_rewrite(args, df)
    print(df.head())
    print(df.shape)
    print(df.columns)

    df.to_csv(f'./data_gen/LLM-texts/rewrite/{args.dataset}_llama_rewrite.csv', index=False)
