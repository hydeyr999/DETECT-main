...
def run(args):
    ...
    df = run_polish(args, df)

    print(df.head())
    print(df.shape)
    print(df.columns)

    df.to_csv(f'./data_gen/LLM-texts/polish/{args.dataset}_llama_polish.csv', index=False)
