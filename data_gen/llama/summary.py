...
def get_response(model,tokenizer,prompt,temperature = 0.7,max_tokens = 3000):
    ...
    with torch.no_grad():
        output_ids = model.generate(
            input_ids,
            max_new_tokens=max_tokens,  # 输出的最大长度
...
def get_summary(model, tokenizer, context):
    prompt_template = summary_prompts[args.dataset]
    prompt = prompt_template.format(context=context)
    response_content = get_response(model, tokenizer, prompt)
    return response_content

def run_summary(args,df):
    tokenizer, model = get_aux_llm(args)
    ai_texts = []
    for _, context in tqdm.tqdm(df.iterrows(), total=len(df)):
        context = context.tolist()[0]
        ai_texts.append(get_summary(model, tokenizer, context))
    df['ai'] = ai_texts
    return df

def run(args):
    df = get_df(args)[2500:2600]

    df = run_summary(args, df)
    print(df.head())
    print(df.shape)
    print(df.columns)

    df.to_csv(f'./data_gen/LLM-texts/summary/{args.dataset}_llama_summary.csv', index=False)
